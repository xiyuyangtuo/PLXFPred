...
def save_to_pickle(dataset: List[Dict], output_dir: str) -> None:
    output_file_path = os.path.join(output_dir, "cord_val.pkl")
    try:
...
def parse_index_file(file_path: str) -> List[Tuple]:
    try:
        ...
        # Check only necessary fields
        required_cols = ["Amino_Acid_Sequence", "ligand_smiles"]
        if len(df.columns) < 5 or not all(col in df.columns for col in required_cols):
        ...
        regression_label_idx = df.columns.get_loc("regression_label")
        classification_label_idx = df.columns.get_loc("classification_label")
...
def process_single_pdb(amino_acid_sequence, ligand_smiles, id=None, regression_label=np.nan, classification_label=np.nan, ligand_preprocessor=None, protein_data=None) -> Optional[ProteinLigandData]:
    try:
        ...
        data_kwargs = {
            ...
            "lig_smiles": standardized_smiles,
...
if __name__ == "__main__":
    logger.info("Starting to build the dataset...")
    ...
    for row in tqdm(index_data, desc="Processing PDB files", dynamic_ncols=True):
        ...
        # Check labels: at least one label (regression_label or classification_label) must not be NaN
        if not (pd.notna(regression_label) or pd.notna(classification_label)):
            logger.warning(f"PDB {pdb_id or 'Unknown'} missing regression_label and classification_label, skipping")
            continue
    ...
    if not dataset:
        ...
        dataloader = DataLoader(pyg_dataset, batch_size=32, shuffle=True)
        logger.info(f"Dataset construction complete, successfully processed {len(dataset)} PDBs.")